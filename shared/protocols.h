...
// ==================== 图像识别结果 ====================
typedef struct {
    bool has_disease;               // 是否检测到病虫害
    char disease_name[32];          // 病虫害名称
    float confidence;               // 置信度 (0-1)
    uint8_t severity;               // 严重程度 (0-5)
    char image_url[128];            // 图片URL (云端存储)
} __attribute__((packed)) image_result_t;

// ==================== 语音指令类型 ====================
#define VOICE_CMD_RELAY     0x01    // 继电器控制
#define VOICE_CMD_QUERY     0x02    // 查询数据
#define VOICE_CMD_SCENE     0x03    // 场景触发
#define VOICE_CMD_STATUS    0x04    // 状态报告

typedef struct {
    uint8_t cmd_type;               // 指令类型
    uint8_t target_dev;             // 目标设备 (0xFF=所有)
    uint8_t param1;                 // 参数1
    uint8_t param2;                 // 参数2
    char text[64];                  // 原始语音文本
} __attribute__((packed)) voice_cmd_t;
...
// 外部 Zigbee 设备的 DPID 映射 (涂鸦标准)
#define ZDPID_SWITCH            1   // 开关 (BOOL)
#define ZDPID_BRIGHTNESS        2   // 亮度 (VALUE 0-100)
#define ZDPID_COLOR_TEMP        3   // 色温 (VALUE 2700-6500)
#define ZDPID_MODE              4   // 模式 (VALUE)

typedef struct {
    uint8_t scene_id;               // 场景ID (1-32)
    bool enabled;                   // 是否启用

    uint8_t trigger_type;           // 0=设备继电器状态, 1=传感器阈值

    // 触发源设备
    uint8_t trigger_dev;            // 源设备 ID
    uint8_t trigger_dpid;           // 触发数据点 (继电器位/传感器DPID)

    // 触发条件
    union {
        struct {
            uint8_t relay_bit;      // 继电器位
            uint8_t state;          // 0=关闭时触发, 1=开启时触发
        } relay;
        struct {
            int32_t threshold_min;  // 下限
            int32_t threshold_max;  // 上限
        } sensor;
    } trigger;

    // 执行动作 - 目标设备
    uint8_t target_type;            // 目标设备类型
    uint8_t target_dev;             // 目标设备 ID
    uint16_t target_ext_addr;       // 外部设备 Zigbee 短地址 (target_type=1 时)
    uint8_t target_ep;              // 外部设备端点 (target_type=1 时)
    uint8_t action_dpid;            // 操作的 DPID
    uint8_t action_value_type;      // 值类型: 1=BOOL, 4=VALUE
    uint32_t action_value;          // 操作值
    uint8_t action_relay_mask;      // 内部设备继电器掩码
    uint8_t action_relay_state;     // 内部设备继电器状态

    uint8_t cooldown_sec;           // 冷却时间 (秒)
    uint32_t last_triggered;        // 上次触发时间戳
} __attribute__((packed)) cross_scene_t;
